...
Blockly.BlockSvg.prototype.renderDraw_ = function(iconWidth, inputRows) {
  ...
  this.pathObject.svgPath.setAttribute('d', pathString);
  this.pathObject.svgPathDark.setAttribute('d', pathString);
  pathString = highlightSteps.join(' ') + '\n' + highlightInlineSteps.join(' ');
  this.pathObject.svgPathLight.setAttribute('d', pathString);
  if (this.RTL) {
    // Mirror the block's path.
    this.pathObject.svgPath.setAttribute('transform', 'scale(-1 1)');
    this.pathObject.svgPathLight.setAttribute('transform', 'scale(-1 1)');
    this.pathObject.svgPathDark.setAttribute('transform', 'translate(1,1) scale(-1 1)');
  }
};
  ...
  for (var y = 0, row; row = inputRows[y]; y++) {
    ...
    if (this.isCollapsed()) {
      ...
      if (this.RTL) {
        ...
        // Short highlight glint at bottom of tab.
        var glintX = (row.subtype == Blockly.INDENTED_VALUE ?
          cursorX + input.fieldWidth + Blockly.BlockSvg.TAB_WIDTH :
          inputRows.rightEdge) - 5;
